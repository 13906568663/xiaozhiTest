...
export default function ChatbotCreatePage() {
  ...
  const loadCapabilities = useCallback(async () => {
    try {
      const [capsRes, kbRes, skillsRes] = await Promise.allSettled([
  ...
  return (
    <div className="flex flex-col gap-5 bg-white px-7 py-6">
      <PageHeader
        title="新建机器人"
        breadcrumb={[{ label: "对话配置" }, { label: "新建机器人" }]}
        actions={
      ...
      <div className="flex gap-4">
        ...
        <div className="flex flex-1 flex-col gap-4">
            ...
            <div className="flex flex-col gap-4">
              ...
              <FormField label="描述">
                <Textarea
                  ...
                  placeholder="一句话概括机器人的职责、面向对象或服务场景"
                />
              </FormField>
              <div className="flex gap-4">
                <FormField label="最大对话轮次" className="w-40" error={errors.max_turns}>
                  <Input
                    ...
                    onChange={(e) => set("max_turns", parseInt(e.target.value) || 50)}
                  />
                </FormField>
                    ...
                    {ICON_STYLES.map((item) => (
                      <button
                        ...
                        className={cn(
                          ...
                          form.icon === item.icon
                            ? "border-2 border-[var(--el-primary)]"
                            : "border-[var(--el-border-lighter)] hover:border-[var(--el-primary)]",
                        )}
                        style={{ backgroundColor: item.bg }}
                        onClick={() => set("icon", item.icon)}
                        title={item.label}
                      >
                        {item.icon}
                      </button>
          ...
          <section className="rounded-xl border border-[var(--el-border-lighter)] bg-white p-5">
            ...
            <p className="mb-3 text-xs text-[var(--el-text-placeholder)]">
              定义机器人角色、行为和回复风格，模型会严格遵守此提示词指导。
            </p>
            <Textarea
              className="min-h-[240px] bg-[#F9F9F9]"
              value={form.system_prompt}
              onChange={(e) => set("system_prompt", e.target.value)}
              placeholder="写清楚机器人的职责、禁止事项、回答风格和工具使用原则。"
            />
            <p className="mt-1.5 text-right text-[11px] text-[var(--el-text-placeholder)]">
              已输入 {form.system_prompt.length} / 2000
            </p>
          </section>
        </div>
        ...
        <div className="flex flex-1 flex-col gap-4">
                ...
                {models.length > 0 ? (
                  <Select
                    ...
                    onChange={(e) => {
                      ...
                      const defModel = provider
                        ? String(provider.config_json.model_name ?? "") || extractAvailableModels(provider.config_json)[0] || ""
                        : "";
                  ...
                  <p className="rounded-lg border border-dashed border-[var(--el-border-lighter)] bg-[#F8FAFF] px-4 py-3 text-sm text-[var(--el-text-secondary)]">
                    暂无可用模型，请先在 Provider 页面创建或启用模型配置。
                  </p>
                )}
              </FormField>
              {form.model_ref && (
          ...
          <section className="rounded-xl border border-[var(--el-border-lighter)] bg-white p-5">
            ...
            {skills.length === 0 ? (
              <p className="rounded-lg border border-dashed border-[var(--el-border-lighter)] bg-[#F8FAFF] px-4 py-3 text-sm text-[var(--el-text-secondary)]">
                暂无可用技能，请先到「技能管理」页面创建并启用 SKILL.md。
              </p>
            ) : (
              <MultiSelect
